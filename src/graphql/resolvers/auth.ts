...
import { IAuthRequest, ILogs } from "../../interfaces/auth.js"
  ...
  Query: {
    logout: async (
      ...
      try {
        ...
        const decodedToken = JSON.parse(result.payload.toString()) as JwtPayload
    ...
    logs: async (_: any, i: any, request: IAuthRequest): Promise<ILogs> => {
      if (!request.isAuth)
        throw new GraphQLError("Invalid Authentication.", {
          extensions: {
            http: {
